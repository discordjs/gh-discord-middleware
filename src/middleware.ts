...
function log(debugLog: boolean, ...messages: unknown[]) {
	if (debugLog) {
...
export default async function middleware(request: Request) {
	...
	if (!secret) {
		log(debugLog, 'No secret set, continuing');
		return new Response(null, { headers: continueHeaders });
	}

	const githubSignature = request.headers.get('X-Hub-Signature-256');
	if (!githubSignature) {
		log(debugLog, 'Secret set, no signature header');
	...
	if (!verified) {
		log(debugLog, 'Verification failed. Local:', await sign(secret, payload), 'Received:', githubSignature);
